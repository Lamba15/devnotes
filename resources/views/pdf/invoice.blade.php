...
<body>
    ...
    for($i=0; $i<18; $i++) {
        $arrowsSvg .= '<polygon points="'.($i*22).',4 '.($i*22 + 9).',9 '.($i*22).',14" fill="none" stroke="#5b377a" stroke-width="2"/>';
    }
    $arrowsSvg .= '</svg>';
    ...
    <div style="position: absolute; top: 85px; right: 50px; font-size: 65px; color: #5b377a; letter-spacing: 4px; font-weight: 200;">
        INVOICE
    </div>

    <div style="position: absolute; top: 184px; left: 70px;">
    ...
    <div style="position: absolute; top: 175px; right: 0; width: 320px; background-color: #9767b8; border-top-left-radius: 8px; border-bottom-left-radius: 8px; padding: 12px 14px; color: #ffffff; font-size: 15px;">
        <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
                <td width="55%" style="font-weight: 600; color: #ffffff;">Invoice: {{ $invoice->reference }}</td>
                <td width="45%" align="right" style="font-weight: 600; color: #ffffff;">{{ $invoice->issued_at?->format('d/m/Y') ?? 'N/A' }}</td>
            </tr>
        </table>
    </div>

    <div style="position: absolute; top: 250px; left: 70px;">
        <div style="font-size: 13px; color: #333; font-weight: bold;">Invoice to:</div>
    ...
    <div style="position: absolute; top: 285px; right: 70px;">
        <svg width="150" height="40" xmlns="http://www.w3.org/2000/svg">
            @for($i=0; $i<10; $i++)
            <line x1="{{ $i*13 }}" y1="40" x2="{{ $i*13 + 30 }}" y2="0" stroke="#9767b8" stroke-width="1.5"/>
            @endfor
        </svg>
    </div>
